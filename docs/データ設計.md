...
erDiagram
    ...
    contracts ||--o{ reviews : "1:N（相互レビュー・最大2件）"
    ...
    reviews {
        bigserial id PK
        bigint contract_id FK "UNIQUE(contract_id, reviewer_role)"
        text reviewer_role "CHECK: company | talent（書いた側）"
        int rating "CHECK: 1〜5。総合評価1軸のみ"
        text comment
        timestamptz submitted_at "★提出した時刻（編集不可）"
        timestamptz published_at "★NULL=相手が未提出のため非公開"
        timestamptz created_at
    }
...
**報復レビューの防止**。提出したら即公開する方式だと、後から書く側が相手の評価を見られ、忖度（★5をもらったから★5を返す）と報復（★2をつけられたから★1）が起きる。

**2つの時刻を分けるだけで実現している**——これがこのテーブルの全部:

| 列 | 意味 |
| --- | --- |
| `submitted_at` | 提出した時刻。**提出＝確定で編集不可** |
| `published_at` | 公開された時刻。**NULL = 相手が未提出のため非公開** |

- `created_at` ひとつだと「提出＝公開」になる。**分けることで「提出されているが公開されていない」状態が生まれる**
- **フラグではなく時刻**にすることで、公開の**事実と時期**が1列で記録でき、`published_at IS NOT NULL` が SELECT でそのまま使える
- **公開はトランザクション内**で行い、**1つの UPDATE で両方の行を更新**する（`WHERE published_at IS NULL`）。片方だけ公開される状態が構造的に作れない
- 未公開の相手レビューは**そもそも SELECT しない**（画面で隠すのではなく取得しない）
- `UNIQUE (contract_id, reviewer_role)` で1契約につき各当事者1件
- **公開期限は設けない**。期限があると「相手が書かないと分かれば安心して悪い評価を書ける」ことになり、報復防止の意味が薄れる
...
このサービスには**状態を持つ仕組みが5つ**ある。すべて「**許可される遷移だけを表に列挙し、判定関数を1つだけ用意する**」という同じ形で実装しているが、**ドメインによって必要な情報が違うため型が変わる**。

| # | 対象 | 状態 | 表の型 | 特徴 |
| --- | --- | --- | --- | --- |
| ① | **応募**（`applications`） | applied / offered / accepted / rejected / withdrawn / declined | `map[遷移]ロール` | **ダブルオプトイン**（accepted への行が1つだけ） |
| — | **案件の掲載**（`projects`） | draft / published / closed | `map[遷移]bool` | 実行者が常に所有企業なのでロール不要。**終端が無い** |
| ② | **契約**（`contracts`） | active / working / reviewing / completed / cancelled | `map[遷移][]ロール` | **差し戻し（戻る遷移）**・中止は双方から実行可 |
| — | **稼働報告**（`work_reports`） | submitted / approved / rejected | `map[遷移][]ロール` | 再提出（戻る遷移）・承認は終端 |
| — | **レビューの公開**（`reviews`） | 未公開 → 公開 | **表ではなく条件判定** | 遷移ではなく「両者が提出したか」の判定 |

- **番号（①②）は取引の進行を表す本筋**。掲載状態・稼働報告・レビューは補助的なもので、番号を振らない（③は M4 の決済に予約されている）
- `statusTransition` 構造体（`{from, to}`）は**すべてで共用**し、**値の型で差を表す**
- **許可だけを列挙する**（ホワイトリスト）。表に無い組み合わせは自動的に拒否され、終端状態は `from` として現れないため遷移できない
- テストは**全状態 × 全遷移先 × 全ロール**を網羅し、**ケース数の検算**も行う（状態を増やしたときの書き漏れを検出する）
...
| contracts | reviews | 1:N（**最大2**） | `UNIQUE(contract_id, reviewer_role)` で各当事者1件 | CASCADE |
...
| reviews | `reviews_contract_reviewer_key` | **UNIQUE**(複合) | **各当事者1件**＋「この契約のレビュー」（先頭列が contract_id なので追加索引は不要） |
...
| テーブル | Phase | 設計の見どころ |
| --- | --- | --- |
| `payments` | M4 | **状態機械③**（お金）。`idempotency_key` で Webhook の重複配送に耐える |
| `notifications` | M4 | 状態遷移の見逃し防止。M3では**ダッシュボードの導線で代替**した（通知テーブルを作ると既読管理・種類の分類・配信先など設計が膨らむため） |
