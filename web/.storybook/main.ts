...
const mockedActionModules = [
  'auth/actions/company',
  'auth/actions/talent',
  'application/actions/create',
  'application/actions/read',
  'application/actions/status',
  'project/actions/read',
  'project/actions/status',
  'projectSearch/actions/read',
  'selection/actions/read',
  'selection/actions/status',
]

const serverActionMocks: Record<string, string> = Object.fromEntries(
  mockedActionModules.map((mod) => [
    `src/features/${mod}.ts`,
    resolve(configDir, `../src/features/${mod}.mock.ts`),
  ]),
)
