...
func main() {
	...
	mux.Handle("GET /me/projects/{id}",
		requireAuth(requireRole(roleCompany)(http.HandlerFunc(handleGetMyProject))))
	...
	// 承認・差し戻しは企業の確認行為
	mux.Handle("PATCH /work-reports/{id}/status",
		requireAuth(requireRole(roleCompany)(http.HandlerFunc(handleUpdateWorkReportStatus))))
	...
	mux.Handle("PATCH /contracts/{id}/status",
		requireAuth(http.HandlerFunc(handleUpdateContractStatus)))
